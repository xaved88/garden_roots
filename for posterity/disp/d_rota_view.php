<?php
	...
	*/
	...
	$staffLib = new staffM( $con, $sql, $param_type=false, $param=false, $group = null, $available = array('start_date'=>$_POST['date'], 'end_date'=>$_POST['date']));
	
	foreach($staffLib->staff as $s){
		$av = $s->is_av($con,$_POST['date'], $_POST['shift_id']);
		if($av && $av > 0){
			$ss = new shortStaff();
