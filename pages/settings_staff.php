...
function save_other(){
	...
	data.email_header = $('textarea#email_header').val();
	data.email_footer = $('textarea#email_footer').val();
	//data.email_css = $('textarea#email_css').val();
	if( $('input#email_cc_sender:checked').length )
		data.email_cc_sender = true;
	data.email_subject = $('textarea#email_subject').val();
	
	data.company_web = new Array();
	if( val = $('select#company_web_0').val() )
		data.company_web.push(val);
	if( val = $('select#company_web_1').val() )
		data.company_web.push(val);
	if( val = $('select#company_web_2').val() )
		data.company_web.push(val);
	$.ajax({
		type:'POST',
		url:'../actions/a_settings.php',
...
<div id='settings_staff'>
	...
	<div class='other'>
		...
		<hr/>
		<div>
			<label for='email_header'>Email Header:</label><br/>
			<textarea name='email_header' id='email_header' col=100 rows=5><?php echo $config->xml->email->header;?></textarea><br/>
			<label for='email_footer'>Email Footer:</label><br/>
			<textarea name='email_footer' id='email_footer' col=100 rows=5><?php echo $config->xml->email->footer;?></textarea><br/>
			<label for='email_subject'>Email Subject:</label><br/>
			<textarea name='email_subject' id='email_subject' col=100 rows=1><?php echo $config->xml->email->subject;?></textarea><br/>
			<label for='email_cc_sender'>CC Sender?</label>
			<input name='email_cc_sender' id='email_cc_sender' type='checkbox' <?php echo $config->xml->email->cc_sender;?>/><br/>
			<span style="font-style:italic; font-size:80%">
				Notes:<br/>
				Table Class: roots_email_table<br/>
				Usable Variables in header/footer/subject: [when][month][year][day][start_date][end_date][name][first_name][last_name]
				All other email options (address, Sending name, IP, etc) are available in the includes/general_functions.php file, search for 'function gt_mail', and look for the appropriate configuartion variables.
			</span>
		</div>
		<hr/>
		Company Web:<br/>
		<?php
			...
			$rota_templates = array();
			foreach($xml->template as $t){
				foreach($t->attributes() as $a => $b){
					array_push($rota_templates, $b);
				}
			}
		?>
		First Display:<select id='company_web_0' name='company_web_0'>
			<option value=''>None</option>
			<?php 
				if(isset($rota_templates) && is_array($rota_templates) && count($rota_templates) > 0)
				foreach($rota_templates as $t){
					$t = trim($t);
					echo "<option value='{$t}'";
					if($t == trim($config->xml->company_web_0)) echo " selected";
					echo ">{$t}</option>";
				}
			?>
		</select><br/>
		Second Display:<select id='company_web_1' name='company_web_1'>
			<option value=''>None</option>
			<?php 
				if(isset($rota_templates) && is_array($rota_templates) && count($rota_templates) > 0)
				foreach($rota_templates as $t){
					$t = trim($t);
					echo "<option value='{$t}'";
					if($t == trim($config->xml->company_web_1)) echo " selected";
					echo ">{$t}</option>";
				}
			?>
		</select><br/>
		Third Display:<select id='company_web_2' name='company_web_2'>
			<option value=''>None</option>
			<?php 
				if(isset($rota_templates) && is_array($rota_templates) && count($rota_templates) > 0)
				foreach($rota_templates as $t){
					$t = trim($t);
					echo "<option value='{$t}'";
					if($t == trim($config->xml->company_web_2)) echo " selected";
					echo ">{$t}</option>";
				}
			?>
		</select><br/>	
