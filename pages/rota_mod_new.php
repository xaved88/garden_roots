...
function applyInfo(data){
	...
	*/
	...
	// APPLY STAFF TYPES
	$(data['staff_type']).each(function(){
		$('table#calendar tbody th[data-staff='+this['staff_id']+']').attr('data-staff_type',this['type']);
	});
...
function orderTest(desc){ // this is actually sort by name, deal with it later.
	prepOrder();
	if(!desc) desc = false;
	tr = $("table#calendar tbody tr");
	
	compare = [];
	data = [];
	tr.each(function(index,value){
		compare[index] = $(value).find('th div.staff_name').html();
		data[index] = $(this).html();
	});
	order = getOrder(compare,desc);

	reOrder(tr,data,order);
}
function orderStaffType(rev){ // rev will need to go and be replaced with order. good enough for now though.
	prepOrder();
	tr = $("table#calendar tbody tr:not(.sort-break)");
	group = [];
	group_order = [2,1];
	if(rev)
	group_order = [1,2];
	...
	$.each(group_order,function(){
		type = this;
		g = [];
		tr.each(function(index,value){
			if($(value).find('th[data-staff_type='+type+']').length)
				g.push(index);
		});
		group.push(g);
	});
	...
	$.each(group,function(index,value){
		spot += this.length - 1;
		addSortBreak(tr[spot]);
	});
}

function orderPos(pos_id){ // NOT STARTED

}
function orderAv(desc){ // NOT STARTED

}

function getOrder(compare, desc){ // alphabetizing
	order = [];
	sorted = compare.slice(0);
	sorted.sort();
...
function reOrder(object, data, order){ // object = jquery object, data = array order = array
	for(i=0; i<order.length; i++){
		$(tr[i]).html(data[order[i]]);
	}
...
function prepOrder(){
	$("table#calendar tr.sort-break").remove();
}
function addSortBreak(row){ // after this row
	$(row).after("<tr class='sort-break'></tr>");
}
...
<div id='controls'>
	...
	<div id='sort_controls'>
		<button onclick=orderTest();>Name Asc</button>
		<button onclick=orderTest(true);>Name Desc</button>
		<button onclick=orderStaffType();>Employees</button>
		<button onclick=orderStaffType(true);>Volunteers</button>
	</div>
	<!--
...
<script src="../js/rota_mod_rules.js"></script>
</body>
