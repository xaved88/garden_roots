...
<html>
<head>
<title>Garden Roots - Rota Templates</title>
...
<style>
	...
	table#mp_template_table, table#cell_seen, div.td_button{
		cursor:pointer;
	...
	div#mp_shifts, div#mp_days, div#mp_css, div#mp_headfoot, div#info_overflow{display:none; padding 3px 8px; border: 1px solid black;}
...
function save(){
	...
	table.show_title = $("input#show_title:checked").val();
	table.title_text = $("input#title_text").val();
	table.title_date_format = $("select#title_date_format").val();
	table.title_date_position = $("select#title_date_position").val();
	...
	table.header_content = $("div#mp_headfoot textarea#header_content_in").val();
	table.header_style = $("div#mp_headfoot textarea#header_style_in").val();
	table.footer_content = $("div#mp_headfoot textarea#footer_content_in").val();
	table.footer_style = $("div#mp_headfoot textarea#footer_style_in").val();
	console.log(table);
	...
	$("div#info_define div.info_inst").each(function(){
		...
		inst.shift_overflow = $(this).find('span.shift_overflow').html();
		inst.name_delim_string = $(this).find('span.name_delim_string').html();
		inst.pos_replicate = $(this).find('span.pos_replicate').html();
		info.push(inst);
	});
	data.info = info;
	}
	...
	$.ajax({
		...
		success:function(data){
			...
			$(data['info']).each(function(){
				...
				html += "<span class='shift_overflow'>" + this['shift_overflow'] + "</span>";
				html += "<span class='name_delim_string'>" + this['name_delim_string'] + "</span>";
				html += "<span class='pos_replicate'>" + this['pos_replicate'] + "</span>";
			...
			if(data['table']['show_title']=='yes')
				$("input#show_title").prop('checked',true);
			else
				$("input#show_title").prop('checked',false);
			$('input#title_text').val(data['table']['title_text']);
			$('select#title_date_format').val(data['table']['title_date_format']);
			$('select#title_date_position').val(data['table']['title_date_position']);
			$('textarea#header_content_in').val(data['table']['header_content']);
			$('textarea#header_style_in').val(data['table']['header_style']);
			$('textarea#footer_content_in').val(data['table']['footer_content']);
			$('textarea#footer_style_in').val(data['table']['footer_style']);
...
function delete_template(){
	...
	if(confirm("Really delete template '" + name + "'?"))
		$.ajax({
			...
			success: function(data){
				$('#action_response').html(data);
				alert("Successfully deleted. Reloading the page.");
				console.log(data);
				location.reload();
			},
			error: function(data){
				$('#action_response').html("Error, could not delete. Logging to console.");
				console.log(data);
			}
		});
...
function toggle_layout(selector){
	selector = "mp_" + selector;
	toggle_div(selector);
}
function toggle_div(selector){
	obj = $("div#" + selector);
	if(obj.is(':visible'))
		obj.hide();
	else
...
function info_select(){
	...
	if(name != '0'){
		...
		seen.find('div#info_overflow div.info_overflow_checkbox input').prop('checked',false);
		
		seen.find('input.name_delim_string').val(inst.find('span.name_delim_string').html());
		if(inst.find('span.pos_replicate').html() == 'yes')
			seen.find('input.pos_replicate').prop('checked',true);
		else
			seen.find('input.pos_replicate').prop('checked',false);
		
		shift_overflow = inst.find('span.shift_overflow').html();
		shift_overflow = shift_overflow.split(',');
		$(shift_overflow).each(function(){
			seen.find("div#info_overflow div.info_overflow_checkbox input[value='" + this + "']").prop('checked',true);
		});
		select_match_selected();
	}
	else{
		...
		seen.find('div#info_overflow div.info_overflow_checkbox input').prop('checked',false);
		seen.find('input.name_delim_string').val('');
		seen.find('input.pos_replicate').prop('checked',false);
		select_match_selected();
	}
}
function save_info(){
	...
	shift_overflow = new Array();
	seen.find('div#info_overflow div.info_overflow_checkbox input:checked').each(function(){
		shift_overflow.push($(this).val());
	});
	shift_overflow = shift_overflow.join();
	console.log("shift overflow: " + shift_overflow);
	// GENERATE NEW DIV
	html = "<span class='name'>" + name + "</span>";
	...
	html += "<span class='shift_overflow'>" + shift_overflow + "</span>";
	html += "<span class='name_delim_string'>" + seen.find('input.name_delim_string').val() + "</span>";
	html += "<span class='pos_replicate'>" + seen.find('input.pos_replicate:checked').val() + "</span>";
...
function listener(){
	...
	$("button#mp_headfoot_button").click(function(){	toggle_layout("headfoot");	});
	$("button#generate_table").click(function(){	generate_table();	});
	$("table#mp_template_table td").click(function(){	place_cell(this);	});
	$("button#info_overflow_button").click(function(){	toggle_div("info_overflow")	});
...
$(function() {
	...
	$("div#rota_temp_accordion").accordion({
		collapsible:true,
		heightStyle:'content',
		active:false
	});
...
<button onclick=delete_template();>Delete</button>
	
<div id='rota_temp_accordion'>
	...
	<div id='table_management'>
		...
		<div id='mp_layout_options'>
			...
			<button id='mp_headfoot_button' class='toggleable'>Header/Footer</button>
			<div id='mp_shifts'>
			<?php
			...
			<div id='mp_css'>
				...
				Title: <input id='show_title' type='checkbox' value='yes' checked>
				Title Text: <input id='title_text' type='text'><br/>
				Title Date: <select id='title_date_format'>
					<option value='none'>None</option>
					<option value='startDjS'>Start: Mon 1st</option>
					<option value='startMjS'>Start: Jan 1st</option>
					<option value='startMjSY'>Start: Jan 1st 1970</option>
					<option value='rangeDjs'>Range: Mon 1st thru Sat 6th</option>
					<option value='rangeMjS'>Range: Jan 1st thru Jan 6th</option>
					<option value='rangeMjSY'>Range: Jan 1st 1970 thru Jan 6th 1970</option>
				</select>
				Title Date Position: <select id='title_date_position'>
			...
			<div id='mp_headfoot'>
				Header: <textarea rows="4" cols="70" id='header_content_in'></textarea><br/>
				Header Style: <textarea rows="2" cols="70" id='header_style_in'></textarea><br/>
				Footer: <textarea rows="4" cols="70" id='footer_content_in'></textarea><br/>
				Footer Style: <textarea rows="2" cols="70" id='footer_style_in'></textarea>
			</div>
		</div>
	...
	<div id='info_define'>
		...
		<div id='info_seen'>
			...
			Text between names: <input type='text' class='name_delim_string' value=''><br/>
			...
			Replicate Pos for every name? <input type='checkbox' class='pos_replicate' value='yes'><br/>
			...
			<div><button id='info_overflow_button' class='toggleable'>Overflow Shifts</button>
				<div id='info_overflow'>
				<?php
					require_once('../includes/staff_classes.php');
					...
					foreach($shifts->ord as $o){
						$s = $shifts->par[$o['shift_id']];
						echo "<div class='info_overflow_checkbox'><input type='checkbox' value='{$s->shift_id}'><span data-abbr='{$s->abbr}'>{$s->name}</span></div>";
					}
				?>
				</div>
			<div>
