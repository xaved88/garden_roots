...
switch($action){	
	...
	if(! function_exists('info_in_cell')){
	...
	if(! function_exists('make_unique')){
	...
	if(! class_exists('tTable')){
		...
		public function print_it($shift, $pos, $staff){
			$html = "<table class='main_table'><thead>";
			$this->make_title();
			if($this->title){
			...
			$html .= "</tr></thead>";
			$html .= "<tbody>";
			foreach($this->row as $i=>$r){
				$html .= "<tr class='main_tr row_{$i}'>";
				...
				foreach($this->col as $j=>$c){
					...
					$html .= "<td class='main_td col_{$j}' data-{$this->col_type}='{$c}' data-{$this->row_type}='{$r}'>";
	...
	*/
	...
	if(!isset($dont_encode) && !$dont_encode)
		echo json_encode($data);
	break;
	}
	...
	{// REMOVE TEMPLATE
		...
		// ELIMINATES TEMPLATES WITH MATCHING NAMES
		foreach($xml->template as $t){
			foreach($t->attributes() as $a => $b){
				if($a == 'name' && $b == $_POST['name']){
					unset($t[0]);
					break 2;
				}
			}
		...
		if($xml->asXML($file))
			echo "Deleted \"{$_POST['name']}\" successfully: " . date("H:i:s") . "<br/>";
		else
			echo "Error deleting \"{$_POST['name']}\": " . date("H:i:s") . "<br/>";
