...
switch($_POST['action']){
	case 'load':
		...
		// PRINTING IT
		echo "<button onclick=save();>Save</button><button id='toggle_partner_link' class='toggleable pressed'>Partner Link</button>
		<br/>
		...
		foreach($staff->staff as $s){
			if($s->is_here($start_date,$end_date)){
				echo "<tr><th class='days_off_staff' data-staff_id='{$s->staff_id}' data-partner_id='{$s->partner}'>{$s->name}</th>";
				for($i=0; $i<$col_count-1; $i++){
					$day_off = $s->get_day_off($weeks[$i][0]);
					echo "<td><select class='days_off_date' data-week='{$i}'><option value='".NULL_DATE."'></option>";
